...
public class Zoo {
    ...
    private List<Swimming_speed> flyer (){
        List<Swimming_speed> res = new ArrayList<>();
        for (Animal animal:animals) {
            if (animal instanceof Swimming_speed) {
                res.add((Swimming_speed) animal);
            }
        }
        return res;
    }

    public Animal championSwSpeed () {
        List<Swimming_speed> start = flyer();
        Swimming_speed champ = start.get(0);
        for (Swimming_speed flyer:start) {
            if (champ.swimming_Speed()< flyer.swimming_Speed()){
                champ = flyer;
            }
        }
        return (Animal)champ;
